...
| `tr-terms [--min-count N] [--top N] [--write]`                      | Finds source terms the model rendered more than one way and proposes glossary entries. Runs no model. Proposals only — harvesting the model’s own output as agreed terminology would pin its mistakes. **Operator only**                                                                     |
<!-- /GENERATED:tools -->
